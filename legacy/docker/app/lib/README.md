...
This directory now vendors the runtime jars copied from the public `hilltop` Wabacus sample so the legacy app can compile and boot inside Docker.
...
- `CustomerSelect` and `CustomerUpdate` are excluded from the Docker compile list because the public `wabacus-4.3.jar` does not expose the legacy extension APIs they require.
