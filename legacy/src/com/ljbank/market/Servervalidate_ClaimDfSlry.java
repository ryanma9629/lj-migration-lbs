...
public class Servervalidate_ClaimDfSlry {
	...
	public static boolean isClaimRatio(ReportRequest rrequest, String RATIO, Map<String, String> mValues, List<String> lstErrorMessages) throws SQLException {
		...
		String sqlcnt="SELECT count(1) as cnt FROM IBS.T1_DF_SLRY_EMP_RELA WHERE EMP_ID=? and ENT_ID=? and RATIO<>0 and CLAIM_STATUS_ID in ('1','2','5','6','21')";
		...
		try {
			String sql = "SELECT 100-SUM(CASE WHEN EMP_ID<>'000000'  and CLAIM_STATUS_ID in ('1','2','5','6','21') THEN ratio ELSE 0 END) AS inratio FROM IBS.T1_DF_SLRY_EMP_RELA WHERE ENT_ID=?" 
				;
