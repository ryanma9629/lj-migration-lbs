...
public class ClaimVerifyAccServerCallBack implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
	@Override
...
//		rrequest.getWResponse().getMessageCollector().success("数据处理中...",false);//("批量审核通过，数据处理中...");//("批量审核通过，数据处理中...", false);//向前台提示一条信息，这里还可以终止后续处理
		...
		if (lstData == null || lstData.size() == 0) {
			...
			for (Map<String, String> mParams : lstData) {
				...
				System.out.println("页面勾选的凭证号:" + vuch_nbr + "员工号为：" + emp_id);
				...
				try {
					...
					String sql1 = "UPDATE IBS.T1_VUCH_EMP_RELA SET CLAIM_STATUS_ID='21' "
							+ ", FH_VERIFY_DT=?"
							+ ", FH_VERIFY_EMP_ID=? "
							+ ", FH_VERIFY_EMP_NM=? "
					...
					pstmt1.setString(3, user_nm);
					pstmt1.setString(4, user_id);
					pstmt1.setString(5, user_nm);
					pstmt1.setString(6, dts);
					pstmt1.setString(7, vuch_nbr);
					pstmt1.setString(8, emp_id);
