...
/*存款认领校验页面*/
public class InterceptorClaimedInfo extends AbsInterceptorDefaultAdapter {
	...
	public Object beforeLoadData(ReportRequest rrequest, ReportBean rbean, Object typeObj, String sql) {
		...
		if("1".equals(org_lev_id)){//支行用户
			...
			sqlcon = sqlcon+ "and cog.BRANCH_ORG_ID='"+usr_org_id+"' and pog.ORG_LEVEL_ID in ('2') ";
			sqlinner = sqlinner+ "inner join ibs.T9_SP_GROUPID sp on sp.BIZ_TYP=re.BIZ_TYPE_NM and sp.GROUP_ID in ("+userRol+") and sp.SP_TYP='按部门' ";
			
		}else if("3".equals(org_lev_id)){//总行用户
			sqlcon = sqlcon+ "and cog.HEAD_ORG_ID='"+usr_org_id+"' and pog.ORG_LEVEL_ID in ('3') ";
			sqlinner = sqlinner+ "inner join ibs.T9_SP_GROUPID sp on sp.BIZ_TYP=re.BIZ_TYPE_NM and sp.GROUP_ID in ("+userRol+") and sp.SP_TYP='按部门' ";
