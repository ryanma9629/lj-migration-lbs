...
public class BrachExsRationVerPassServerCallBack implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
	@Override
		...
		if (lstData == null || lstData.size() == 0) {
			...
			for (Map<String, String> mParams : lstData) {
				...
				try {
					...
					String sqlPass ="UPDATE IBS.T3_BRANCH_EXS_RATIO SET STUS='3',VERIFY_PEOPLE=?, VERIFY_TIME=?,VERIFY_IP=?, VERIFY_REMARK='审批批量通过。' " +
							"WHERE EXAM_YEAR = ? AND EXAM_Q=? AND BRANCH_ORG_ID=? AND STUS='1' " ;
