...
public class ClaimVerifyExsFhServerCallBack implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
	@Override
		...
		if (lstData == null || lstData.size() == 0) {
			...
			for (Map<String, String> mParams : lstData) {
				...
				try {
					...
					String sql = "UPDATE IBS.T1_VUCH_EMP_RELA SET CLAIM_STATUS_ID='2' "
							+ ", VERIFY_DT=?"
							+ ", VERIFY_EMP_ID=? "
							+ ", REMARK=CONCAT(COALESCE(REMARK,''),'审批时间',?,'审批工号',?,'审批理由页面批复存量客户分行审核通过。') "
