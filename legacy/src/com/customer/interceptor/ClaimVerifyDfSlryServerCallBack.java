...
public class ClaimVerifyDfSlryServerCallBack implements IServerAction {
		...
		if (lstData == null || lstData.size() == 0) {
			...
			for (Map<String, String> mParams : lstData) {
				for (Entry<String, String> entryTmp : mParams.entrySet()) {
					if (entryTmp.getKey().equals("AGENT_NO")) {
						agent_no = entryTmp.getValue();
					}
					if (entryTmp.getKey().equals("EMP_ID")) {
						emp_id = entryTmp.getValue();
					}
					if (entryTmp.getKey().equals("CLAIM_DT")) {
						claim_dt = entryTmp.getValue();
					}
				}
				...
				try {
					...
					String sql = "UPDATE IBS.T1_DF_SLRY_EMP_RELA SET CLAIM_STATUS_ID='2' "
							+ ", VERIFY_DT=?"
							+ ", VERIFY_EMP_ID=? "
							+ ", REMARK=CONCAT(COALESCE(REMARK,''),'审批时间',?,'审批工号',?,'审批理由页面批复通过。') "
					...
					pstmt.setString(5, agent_no);
					pstmt.setString(6, emp_id);
					pstmt.setString(7, claim_dt);
					pstmt.executeUpdate();
