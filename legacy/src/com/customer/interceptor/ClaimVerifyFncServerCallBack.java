...
public class ClaimVerifyFncServerCallBack implements IServerAction {
		...
		if (lstData == null || lstData.size() == 0) {
			...
			for (Map<String, String> mParams : lstData) {
				for (Entry<String, String> entryTmp : mParams.entrySet()) {
					if (entryTmp.getKey().equals("TXN_NO")) {
						txn_nbr = entryTmp.getValue();
					}
				...
				try {
					...
					String sql = "UPDATE IBS.T1_FNC_TXN_EMP_RELA SET CLAIM_STATUS_ID='2' "
							+ ", VERIFY_DT=?"
							+ ", VERIFY_EMP_ID=? "
