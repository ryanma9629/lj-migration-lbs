...
public class EmpDxFhScoSubmitServerCallBack implements IServerAction {
...
//		String dt = dateformat.format(date);
		...
		if (lstData == null || lstData.size() == 0) {
			...
			for (Map<String, String> mParams : lstData) {
				...
				String sqlSub ="UPDATE IBS.T2_EMP_DX_SCORE SET STUS='2' ,OU_MOD_PEOPLE=? ,OU_MOD_TIME=? ,OU_MOD_IP=?, SCORE=? " +
				"WHERE STUS='1' AND EXAM_YEAR=? AND EXAM_Q=? AND MSR_PRD_DT=? AND PROJCT_NM=? AND ZB_NM=? and EMP_ID=? " ;
