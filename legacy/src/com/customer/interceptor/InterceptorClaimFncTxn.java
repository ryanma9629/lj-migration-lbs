...
/*基金认领*/
public class InterceptorClaimFncTxn extends AbsInterceptorDefaultAdapter {

	@Override
	public void doStart(ReportRequest rrequest, ReportBean rbean) {
		String txn_dtSt = rrequest.getStringAttribute("TXN_DT_ST", "");
		String txn_dtEnd = rrequest.getStringAttribute("TXN_DT_END", "");
		String vuch_nbr = rrequest.getStringAttribute("VUCH_NBR", "");
		...
		if(txn_dtSt==null || txn_dtEnd==null || txn_dtSt.equals("") || txn_dtEnd.equals("")){
			...
			rrequest.setAttribute("list_lazyloadmessage", "现在是延迟读取，输入交易日期+流水号/凭证号后点击查询按钮才加载相应条件的数据");
			rrequest.authorize("list", Consts.BUTTON_PART, "sub", "disabled", "true");
		}else if((vuch_nbr==null || vuch_nbr.equals("")) && (txn_no==null || txn_no.equals("")) ){
			rrequest.setAttribute("list_lazyload", "true");
			rrequest.setAttribute("list_lazyloadmessage", "现在是延迟读取，输入交易日期+流水号/凭证号后点击查询按钮才加载相应条件的数据");
			rrequest.authorize("list", Consts.BUTTON_PART, "sub", "disabled", "true");
		}
