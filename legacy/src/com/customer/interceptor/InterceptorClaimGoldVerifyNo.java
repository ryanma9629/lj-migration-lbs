...
/*认领审核不通过弹窗页面*/
public class InterceptorClaimGoldVerifyNo extends AbsInterceptorDefaultAdapter {
	...
	public void doStart(ReportRequest rrequest, ReportBean rbean) {
		...
		String txn_nbr = rrequest.getStringAttribute("TXN_RUN_NBR", "");     // 取出认领人工号
		
		Connection conn = Config.getInstance().getDataSource("ds_db2").getConnection();// 取DB2数据源配置
		
		if (!txn_nbr.equals("")) {

			String SQL="Select count(1) as cnt from IBS.GOLD_EMP_RELA where TXN_RUN_NBR=? and EMP_ID=? and TXN_DT=? and CLAIM_STATUS_ID='3' ";
