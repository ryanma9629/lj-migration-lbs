		...
		try {
			...
			if(cnt>=1){
				...
				try { // 提交按钮补充校验判断凭证认领剩余比例
					String sqlvalidate = "SELECT COALESCE(SUM(CASE WHEN EMP_ID=? and CLAIM_STATUS_ID='0' THEN ratio ELSE 0 END ),0) AS ro,100-SUM(CASE WHEN EMP_ID<>'000000'  and CLAIM_STATUS_ID in ('1','2','5','6') THEN ratio ELSE 0 END) AS inratio FROM IBS.T1_FNC_TXN_EMP_RELA WHERE TXN_NO=? and TXN_DT=? with ur";
					...
					if (ro > inratio) {
						...
						String sqlUpd= "update IBS.T1_FNC_TXN_EMP_RELA set CLAIM_STATUS_ID='1', CLAIM_DT=?, REMARK='认领时间'||?||'认领工号'||?||'认领理由'||REMARK1, REMARK1='' ,EMP_NM=?,EMP_ORG_ID=?, EMP_ORG_NM=? where TXN_NO=? and EMP_ID=? and TXN_DT=? ";
