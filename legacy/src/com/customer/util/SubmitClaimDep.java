...
/*存款业绩认领提交按钮方法*/
public class SubmitClaimDep implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
	@Override
	...
	public String executeSeverAction(ReportRequest rrequest, IComponentConfigBean ccbean, List<Map<String, String>> lstData,
		...
		try {
			...
			System.out.println("sqlBrch="+sqlBrch);
			...
			if(user_id.equals(usr_org_id) && (rsBrch.getString("ORG_LEVEL_ID")).equals("2") && rsBrch.getString("BIZ_TYPE_NM").equals("对公存款")){
				...
				String sqlvalidate="select sum(ro) ro,sum(inratio) inratio " +
						...
						"union all " +
						"select 0 as ro, ratio as inratio FROM IBS.T1_VUCH_EMP_RELA r inner join ibs.T5_ORG og on og.ORG_ID=r.EMP_ID and og.ORG_LEVEL_ID='2' and og.ORG_TYPE_ID='EO' where r.VUCH_NBR=? and CLAIM_STATUS_ID in ('1','2','5','6') and BIZ_TYPE_NM='对公存款'" +
						")";
				...
				//System.out.println("分行用户已认领的比例BraRo============" + BraRo);
				double inratio = rs.getDouble("inratio");
				System.out.println("员工剩余可以认领的比例inratio============" + inratio);
