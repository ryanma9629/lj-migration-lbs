...
/*基金认领提交按钮方法*/
public class SubmitClaimFundTxn implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
	@Override
	...
	public String executeSeverAction(ReportRequest rrequest, IComponentConfigBean ccbean, List<Map<String, String>> lstData,
		...
		String inpRo = (String)rrequest.getRequest().getSession().getAttribute("RATIO_DFAULT");
		...
		try {
			...
			if(cnt>=1){
				...
				try { // 提交按钮补充校验判断凭证认领剩余比例
					String sqlvalidate = "SELECT 100-coalesce(SUM(ratio),0) AS inratio FROM IBS.T1_FUND_TXN_EMP_RELA WHERE TXN_NO=? and TXN_DT=? and CLAIM_STATUS_ID in ('1','2','5','6','21')";
					
					PreparedStatement pstmt0 = null;
					pstmt0 = conn.prepareStatement(sqlvalidate);
					pstmt0.setString(1, txn_no);
					pstmt0.setString(2, txn_dt);
					ResultSet rs = pstmt0.executeQuery();
					rs.next();

					double inratio = rs.getDouble("inratio"); //剩作比例
					double ro = Double.parseDouble(inpRo);//输入的比例
					...
					if (ro > inratio) {
						...
						String sqlIsSave="select count(1) as cnt from IBS.T1_FUND_TXN_EMP_RELA where TXN_NO=? and TXN_DT=? and EMP_ID=? and CLAIM_STATUS_ID='0'";
						
						PreparedStatement pstmtIs = null;
						pstmtIs = conn.prepareStatement(sqlIsSave);
						pstmtIs.setString(1, txn_no);
						pstmtIs.setString(2, txn_dt);
						pstmtIs.setString(3, user_id);
						ResultSet rsIs = pstmtIs.executeQuery();
						rsIs.next();
						
						int isSave = rsIs.getInt("cnt");
						...
						String sqlUpd= "update IBS.T1_FUND_TXN_EMP_RELA set CLAIM_STATUS_ID='1', CLAIM_DT=?, REMARK=CONCAT('认领时间',?,'认领工号',?,'认领理由',COALESCE(REMARK1,'')),EMP_NM=?,EMP_ORG_ID=?, EMP_ORG_NM=? where TXN_NO=? and TXN_DT=? and EMP_ID=? ";
						...
						pstmt.setString(7, txn_no);
						pstmt.setString(8, txn_dt);
						pstmt.setString(9, old_emp_id);
						pstmt.executeUpdate();
