...
/*基金定投认领提交按钮方法*/
public class SubmitClaimFundDt implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
	@Override
		...
		try {
			...
			if(cnt>=1){
				...
				try { // 提交按钮补充校验判断凭证认领剩余比例
					String sqlvalidate = "SELECT 100-coalesce(SUM(ratio),0) AS inratio FROM IBS.T1_FUND_DT_EMP_RELA WHERE SERIAL_NO=? and CLAIM_STATUS_ID in ('1','2','5','6','21')";
					
					PreparedStatement pstmt0 = null;
					pstmt0 = conn.prepareStatement(sqlvalidate);
					pstmt0.setString(1, ser_no);
					...
					if (ro > inratio) {
						...
						String sqlIsSave="select count(1) as cnt from IBS.T1_FUND_DT_EMP_RELA where SERIAL_NO=? and EMP_ID=? and CLAIM_STATUS_ID='0'";
						
						PreparedStatement pstmtIs = null;
						pstmtIs = conn.prepareStatement(sqlIsSave);
						pstmtIs.setString(1, ser_no);
						pstmtIs.setString(2, user_id);
						...
						String sqlUpd= "update IBS.T1_FUND_DT_EMP_RELA set CLAIM_STATUS_ID='1', CLAIM_DT=?, REMARK=CONCAT('认领时间',?,'认领工号',?,'认领理由',COALESCE(REMARK1,'')), EMP_NM=?,EMP_ORG_ID=?, EMP_ORG_NM=? where SERIAL_NO=? and EMP_ID=? ";
