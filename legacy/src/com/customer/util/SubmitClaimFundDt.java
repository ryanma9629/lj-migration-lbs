		...
		try {
			...
			if(cnt>=1){
				...
				try { // 提交按钮补充校验判断凭证认领剩余比例
					String sqlvalidate = "SELECT 100-coalesce(SUM(ratio),0) AS inratio FROM IBS.T1_FUND_DT_EMP_RELA WHERE SERIAL_NO=? and CLAIM_STATUS_ID in ('1','2','5','6','21') with ur";
					...
					if (ro > inratio) {
						...
						String sqlUpd= "update IBS.T1_FUND_DT_EMP_RELA set CLAIM_STATUS_ID='1', CLAIM_DT=?, REMARK='认领时间'||?||'认领工号'||?||'认领理由'||REMARK1, EMP_NM=?,EMP_ORG_ID=?, EMP_ORG_NM=? where SERIAL_NO=? and EMP_ID=? ";
